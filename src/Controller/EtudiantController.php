...
class EtudiantController extends BaseController
{
    ...
    public function store(Request $request, array $params = []): void
    {
        ...
        $this->response->redirect('/etudiants/' . $id);
    }
    ...
    private function validate(array $data, ?int $id = null): array
    {
        ...
        if ($data['prenom'] === '' || !Validator::maxLen($data['prenom'], 50)) { $errors['prenom'] = 'Prénom requis (<=50).'; }
        if (!Validator::email($data['email']) || !Validator::maxLen($data['email'], 100)) { $errors['email'] = 'Email invalide (<=100).'; }
        if ($data['filiere_id'] <= 0 || !$this->filiereDao->findById((int)$data['filiere_id'])) { $errors['filiere_id'] = 'Filière invalide.'; }
            return $errors;
        }
